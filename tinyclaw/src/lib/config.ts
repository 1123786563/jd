...
import { Settings, AgentConfig, TeamConfig, CLAUDE_MODEL_IDS, CODEX_MODEL_IDS, OPENCODE_MODEL_IDS, ZHIPU_MODEL_IDS, KIMI_MODEL_IDS, QWEN_MODEL_IDS } from './types';
...
/**
 * Resolve the model ID for Zhipu (智谱).
 */
export function resolveZhipuModel(model: string): string {
    return ZHIPU_MODEL_IDS[model] || model || '';
}

/**
 * Resolve the model ID for Kimi (Moonshot).
 */
export function resolveKimiModel(model: string): string {
    return KIMI_MODEL_IDS[model] || model || '';
}

/**
 * Resolve the model ID for Qwen (通义千问).
 */
export function resolveQwenModel(model: string): string {
    return QWEN_MODEL_IDS[model] || model || '';
}
