...
import { SCRIPT_DIR, resolveClaudeModel, resolveCodexModel, resolveOpenCodeModel, resolveZhipuModel, resolveKimiModel, resolveQwenModel } from './config';
...
export async function invokeAgent(
    ...
    if (provider === 'openai') {
        ...
        // 智谱 AI (GLM) - 使用 opencode CLI 调用
        const modelId = resolveZhipuModel(agent.model);
        log('INFO', `Using Zhipu AI (智谱) (agent: ${agentId}, model: ${modelId})`);

        const continueConversation = !shouldReset;

        if (shouldReset) {
            log('INFO', `🔄 Resetting Zhipu conversation for agent: ${agentId}`);
        }
        ...
        return response || 'Sorry, I could not generate a response from Zhipu AI.';
    } else if (provider === 'kimi' || provider === 'moonshot') {
        // Kimi (Moonshot) - 使用 opencode CLI 调用
        const modelId = resolveKimiModel(agent.model);
        log('INFO', `Using Kimi (Moonshot) (agent: ${agentId}, model: ${modelId})`);
        ...
        return response || 'Sorry, I could not generate a response from Kimi.';
    } else if (provider === 'qwen' || provider === 'tongyi' || provider === 'alibaba') {
        // 通义千问 (Qwen) - 使用 opencode CLI 调用
        const modelId = resolveQwenModel(agent.model);
        log('INFO', `Using Qwen (通义千问) (agent: ${agentId}, model: ${modelId})`);

        const continueConversation = !shouldReset;

        if (shouldReset) {
            log('INFO', `🔄 Resetting Qwen conversation for agent: ${agentId}`);
        }
        ...
        return response || 'Sorry, I could not generate a response from Qwen.';
    } else {
