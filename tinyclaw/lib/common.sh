...
ALL_CHANNELS=(discord whatsapp telegram feishu)
...
declare -A CHANNEL_SCRIPT=(
    ...
    [feishu]="dist/channels/feishu-client.js"
)
...
load_settings() {
    ...
    for ch in "${ALL_CHANNELS[@]}"; do
        ...
        # Feishu needs both app_id and app_secret
        if [ "$ch" = "feishu" ]; then
            CHANNEL_TOKENS[$ch]=$(jq -r ".channels.${ch}.app_id // empty" "$SETTINGS_FILE" 2>/dev/null)
            FEISHU_APP_SECRET=$(jq -r ".channels.${ch}.app_secret // empty" "$SETTINGS_FILE" 2>/dev/null)
            FEISHU_ENCRYPT_KEY=$(jq -r ".channels.${ch}.encrypt_key // empty" "$SETTINGS_FILE" 2>/dev/null)
        fi
    done
