...
ALL_CHANNELS=(telegram discord whatsapp feishu)
...
declare -A CHANNEL_TOKEN_PROMPT=(
    ...
    [feishu]="Enter your Feishu App ID:"
)
declare -A CHANNEL_TOKEN_HELP=(
    ...
    [feishu]="(Get one at: https://open.feishu.cn/app)"
)
...
for ch in "${ENABLED_CHANNELS[@]}"; do
    ...
    if [ "$ch" = "feishu" ]; then
        echo "Enter your Feishu App Secret:"
        echo -e "${YELLOW}(Find it at: https://open.feishu.cn/app -> Your App -> Credentials)${NC}"
        echo ""
        read -rp "App Secret: " feishu_secret_value
        FEISHU_APP_SECRET="$feishu_secret_value"
        echo -e "${GREEN}✓ Feishu App Secret saved${NC}"
        echo ""

        echo "Enter your Feishu Encrypt Key (for webhook signature verification):"
        echo -e "${YELLOW}(Find it at: https://open.feishu.cn/app -> Your App -> Event Subscriptions -> Encrypt Key)${NC}"
        echo -e "${YELLOW}(Can be left empty for development, but recommended for production)${NC}"
        echo ""
        read -rp "Encrypt Key: " feishu_encrypt_key_value
        FEISHU_ENCRYPT_KEY="$feishu_encrypt_key_value"
        if [ -n "$FEISHU_ENCRYPT_KEY" ]; then
            echo -e "${GREEN}✓ Feishu Encrypt Key saved${NC}"
        else
            echo -e "${YELLOW}⚠ Feishu Encrypt Key not set - webhook signature verification will be disabled${NC}"
        fi
        echo ""
    fi
done
