...
Automaton 是一个带有财务意识、能够在区块链上自主注册并管理计算资源的自主智能体（Sovereign AI Agent）框架。通过深度阅读 [index.ts](automaton/src/index.ts) 及 [orchestrator.ts](automaton/src/orchestration/orchestrator.ts)，其底层运作机制如下：

**核心底层机制提取：**

* **生命周期与守护进程 (Lifecycle & Heartbeat)**：由 `heartbeat/daemon.ts` 维持心跳，并抛出唤醒事件（`wake_events`）。核⼼的 [runAgentLoop](automaton/src/agent/loop.ts#90-939) 基于 ReAct 模式管理状态机（`waking -> running -> sleeping -> critical -> dead`）。
* **编排引擎与任务图 (Orchestration & Task Graph)**：[orchestrator.ts](automaton/src/orchestration/orchestrator.ts) 维护了一个复杂的七步状态机（包括 `classifying, planning, executing, replanning`）。它通过调用 `planGoal` 自动将大目标分解为 [TaskNode](automaton/src/orchestration/task-graph.ts#34-57) 子任务树，并可通过 [trySpawnAgent](automaton/src/orchestration/orchestrator.ts#889-919) 机制消耗 Credit 在 Conway 沙盒内动态生成子代理分配工作。
* **经济决策与风控引擎 (Policy Engine)**：所有工具调用成本由 [SpendTracker](automaton/src/types.ts#529-537) 追踪。框架在启动时（[index.ts](automaton/src/index.ts)）计算存款，当资金不足时会调用 `bootstrapTopup` 自动将绑定的 USDC 闪兑为运行学分（Credits）。这受到 `DEFAULT_TREASURY_POLICY` 的硬编码限额约束。
* **Web4 链上交互能力**：在初始化时调用 [client.ts](automaton/src/conway/client.ts) 中的 [registerAutomaton](automaton/src/conway/client.ts#364-435)，将代理的钱包地址及唯一的 `genesisPromptHash` 固化至 EVM 区块链上的 ERC-8004 Registry 智能合约中。
...
TinyClaw 是一个轻量级、高并发的多智能体协同与消息路由系统，侧重于异步消息队列与多端聊天通道的整合。通过深度阅读 [queue-processor.ts](tinyclaw/src/queue-processor.ts), [db.ts](tinyclaw/src/lib/db.ts) 及 [routing.ts](tinyclaw/src/lib/routing.ts)，解析其底层设计如下：

**核心底层机制提取：**

* **事务强一致性消息队列 (SQLite Message Bus)**：[db.ts](tinyclaw/src/lib/db.ts) 抛弃了传统的文件队列，采用 better-sqlite3 配合 WAL 模式。通过在 [claimNextMessage](tinyclaw/src/lib/db.ts#176-202) 方法中使用 `BEGIN IMMEDIATE` 独占事务锁定，确保高并发情况下同一条消息绝不会被重复提取。
* **并行协程锁链 (Per-Agent Promise Chains)**：在 [queue-processor.ts](tinyclaw/src/queue-processor.ts) 中维护的 `agentProcessingChains: Map<string, Promise<void>>` 是一种极其优雅的设计。它确保了分配给**同一个 Agent** 的消息绝对串行（避免上下文串台错乱），而分给**不同 Agent** 的消息则完全并行执行，算力最大化。
* **组内通讯与幽灵状态机 (Team Mentions & Pending Tree)**：[routing.ts](tinyclaw/src/lib/routing.ts) 通过正则提取类似 `[@teammate: message]` 的标签实现 Agent 互相呼叫。内部会通过 [incrementPending](tinyclaw/src/lib/conversation.ts#51-58) 方法挂起主对话的结算期；在有未收到回复的队友时，引擎会自动追加提示词防止主 Agent 反复催促，直到所有对话分支都 [decrementPending](tinyclaw/src/lib/conversation.ts#59-74) 清零才视为任务完成。
...
* 🏗️ **4. ArchitectAgent (系统架构)**
  ...
  * **输出/挂载**: 调取 Automaton 深层的 [decomposeGoal](automaton/src/orchestration/task-graph.ts#117-234) 算法，在数据库生成一颗依赖分明的 [TaskNode](automaton/src/orchestration/task-graph.ts#34-57)（子任务树）。
* 💻 **5. DevAgent (主研开发)**
  * **输入**: [TaskNode](automaton/src/orchestration/task-graph.ts#34-57) 叶子节点的具体编程任务。
...
整合架构的精髓不仅在于“顺风局”的正向流水线，更在于多体互博时的**深层通信、状态寻址与异常回退（自愈）能力**。结合 TinyClaw 底层的 [queue-processor.ts](tinyclaw/src/queue-processor.ts) 与 Automaton 的 [task-graph.ts](automaton/src/orchestration/task-graph.ts)，以下是系统运转的实质引擎：
...
* **@提及路由调度 (@Mention Routing)**：代码层面的交接犹如人类聊天群。例如当 Scout 获取到订单后，会生成如下回复：`"@sales-agent 发现高价值线索，请分析附件JD。 @accountant-agent 请对客户ID进行背景核验。"`。[queue-processor.ts](tinyclaw/src/queue-processor.ts) 会用正则提取这些 mention，利用 [enqueueInternalMessage](tinyclaw/src/lib/conversation.ts#75-98) 帮它们各自建立一个并发的子对话分支（Pending Branch）。
...
* **长文本转 DAG 图 (Task Graph Generation)**：客户的需求文档通常是非结构化的。ArchitectAgent 的核心职责不是写代码，而是调用 Automaton 底层的 [decomposeGoal()](automaton/src/orchestration/task-graph.ts#117-234) 算法。它结合大模型的结构化输出能力，将复杂需求拆解为**有向无环图 (DAG)** 形式的 [TaskNode](automaton/src/orchestration/task-graph.ts#34-57)（包含节点 ID、描述、以及强制必须做完的上一级前置依赖 `dependencies`）。
...
* **多智能体并发组网**：实现 Scout 获取需求后，基于正则 `@sales-agent` 和 `@accountant-agent` 的 SQLite 并发唤醒队列（[queue-processor.ts](tinyclaw/src/queue-processor.ts)）。
...
**⚠️ 技术难点与风险 (Risks)：**
* **并发死锁 (SQLite Locked)**：由于多个 Agent（如 Sales 和 Accountant）会同时被 `@` 并试图竞争写入同一行的 [Conversation](tinyclaw/src/lib/types.ts#79-95) 上下文，极易触发 SQLite 经典的 `SQLITE_BUSY` 死锁。代码需严格实现带有退避算法的写锁 [withConversationLock](tinyclaw/src/lib/conversation.ts#17-50)。

---

### Phase 3: 隔离开发、DAG拆解与极简沙盒 (MVP 跑通) (Weeks 5-6)
**🎯 核心任务：**
* **架构师拆图 (DAG Decomposition)**：开发 Architect Agent，将杂乱长文本拆分为有依赖关系的 DAG 任务树 ([task-graph.ts](automaton/src/orchestration/task-graph.ts))。
